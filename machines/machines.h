...
namespace TailTest {
//static char wrong_size_1[1 + sizeof(std::size_t) - sizeof(std::NumVec<size_t>::size_type)];
//
//static char wrong_size_2[1 - sizeof(std::size_t) + sizeof(std::NumVec<size_t>::size_type)];
...
typedef std::unordered_set<uint32_t> NumSet;
...
struct MacroState{
    ...
    protected:
        ...
        FSM_old(uint32_t in_size, uint32_t out_size) : in_alphabet_size(in_size), out_alphabet_size(out_size),
                                                       size_impl(0) {};
...
//                                                    uint32_t in) const;
    ...
    public:
        ...
        NumVec getOutSeq(const NumVec& seq, uint32_t state=0) const;
